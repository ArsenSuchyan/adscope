(() => {
    ...
    da.init();
    let btn = document.querySelector(".header__navigation");
    ...
    function documentActions(e) {
        ...
        if (target.closest(".header__navigation")) {
            menu.classList.toggle("open");
            btn.classList.toggle("open");
        } else {
            menu.classList.contains("open") ? menu.classList.remove("open") : null;
            btn.classList.contains("open") ? btn.classList.remove("open") : null;
        }
    }
    function showUpButton() {
        if (window.scrollY > 0) upward.style.display = "block"; else upward.style.display = "none";
    }
    const options = {
        root: null,
